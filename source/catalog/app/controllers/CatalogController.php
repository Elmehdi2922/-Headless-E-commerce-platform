...
class CatalogController extends \Ubiquity\controllers\rest\RestController {
    ...
    {
        $c =DAO::getById(Catalog::class,$id);
        if($c != null)
        {
            Catalog::deletebyCatalog($id);
            if(Catalog::deleteCat($id))
    ...
    public function ViderCatalog($id)
    {
        ...
        {
            if(DAO::getAll(CatProd::class,'catalog='.$id,false))
            {
                ...
                echo json_encode(["status"=>"OK 200","title"=>"Catalog $id is already empty !"]);
            }
    ...
    /**
     * @route("/ajouterProd/{idproduct}/ToCatalog/{idcatalog}","methods"=>["post"])
     */
    public function AddProductToCatalog($idcatalog,$idproduct)
    {
        $catpr= new CatProd();

        $catpr->setCatalog($idcatalog);
        $catpr->setProduct($idproduct);
        if(DAO::getAll(CatProd::class,"catalog = ".$idcatalog." and product =".$idproduct))
        {
            echo json_encode(["status"=>"OK 200","title"=>"Product already exist in this catalog!"]);
        }else
        {
            if($catpr->addProdToCatalog())
