...
class SenderClass
{
    ...
    public function form_params_catalog($datas,$mtd)
    {
        ...
        if($mtd==="POST")
            $this->client = new Client(['form_params' => ['libelle' => $dt[libelle],'details' => $dt[details],'image' => $dt[image],'etat' => $dt[etat],'datec' => $dt[datec]]]);
        else if($mtd==="PATCH")
            $this->client = new Client(['form_params' => ['id' => $dt[id],'libelle' => $dt[libelle],'details' => $dt[details],'image' => $dt[image],'etat' => $dt[etat],'datec' => $dt[datec]]]);
        else
            $this->client = new Client();
